...
async function runLoop(
	...
	const MAX_NO_TOOL_RETRIES = 4;  // v26: more retries to prevent 0-output
	...
	const TIME_WARNING_MS = 50_000;  // v26: warn earlier  // 80s — inject "time running out"
						...
						content: [
							{
								...
								text: isLength
									...
									: "0 lines = loss. Call `read` on the file most related to the task, then `edit` it. Do it NOW.",
							},
						],
						timestamp: Date.now(),
					});
					...
					if (tr.isError) {
						...
						pendingMessages.push({
							...
							content: [
								{
									type: "text",
									text: `${targetPath} saved. Re-read before re-editing.`,
								},
							],
							timestamp: Date.now(),
						});
				...
				if (!hasEditedAnyFile && !timeWarningInjected && (Date.now() - loopStartTime) >= TIME_WARNING_MS && pendingMessages.length === 0) {
					...
					pendingMessages.push({
						...
						content: [
							{
								type: "text",
								text: "50s, 0 edits. Call `edit` on ANY file NOW. Best guess beats 0.",
							},
						],
						timestamp: Date.now(),
					});
