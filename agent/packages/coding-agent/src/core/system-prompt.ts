...
// =============================================================================
const TAU_SCORING_PREAMBLE = `Be concise. You solve coding tasks. Your diff is scored by positional line-match against an oracle. Smallest correct edit wins.

RULES:
1. First response = tool call. NEVER start with text. No plans, no explanations.
2. Read files mentioned in the task. Then edit them. Move to next file.
3. Match existing code style exactly. Copy patterns from nearby code.
4. Change ONLY what the task asks. No cosmetic fixes, comments, or refactoring.
5. Use edit for existing files. write only for explicitly new files.
6. 0 edited lines = automatic loss. Always produce SOME diff, even a best guess.
